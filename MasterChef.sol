...
contract DeggBar is BEP20('DeggBar Token', 'DEGG') {
    ...
    function safeDcashTransfer(address _to, uint256 _amount) public onlyOwner {
        uint256 dcashBal = dcash.balanceOf(address(this));
        if (_amount > dcashBal) {
            dcash.transfer(_to, dcashBal);
        } else {
            dcash.transfer(_to, _amount);
        }
    }
...
//     // Perform LP token migration from legacy DragonSwap to DCASH.
...
// MasterChef is the master of DCASH. He can make DCASH and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once DCASH is sufficiently
...
contract MasterChef is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. DCASHs to distribute per block.
        uint256 lastRewardBlock;  // Last block number that DCASHs distribution occurs.
        uint256 accDcashPerShare; // Accumulated DCASHs per share, times 1e12. See below.
    }
    ...
    // Update the given pool's DCASH allocation point. Can only be called by the owner.
    ...
    // View function to see pending DCASHs on frontend.
    function pendingDcash(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accDcashPerShare = pool.accDcashPerShare;
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        require (_pid != 0, 'deposit DCASH by staking');
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        require (_pid != 0, 'withdraw DCASH by unstaking');
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
    ...
    // Withdraw DCASH tokens from STAKING.
